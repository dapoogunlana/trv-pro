  ...
  export const getStayToAndFrom = (stay: any) => {
    let from = stay.outbound[0]?.airport_from_details?.city || stay.outbound[0]?.airport_from
    let to = stay.outbound[stay.outbound.length - 1]?.airport_to_details?.city || stay.outbound[stay.outbound.length - 1]?.airport_to
    return {from, to}
  }
