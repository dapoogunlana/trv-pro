...
import { activateAllTouchFields, stayPaymentInitialData, IStayBookingPayload, IStayBookingTouchedPayload, IStayPaymentData, initialContactPayload, initialContactTouchedData, initialStayBookingPayload, initialPassengerPayload, initialPassengerTouchedData, IPassengerPayload, updateBookingData, validateForErrors } from './stay-booking-service';
import StayPayment from './stay-payment/stay-payment';
...
function StayBookingPage(props: any) {
  ...
  const [stayDetails, setStayDetails] = useState<any>({outbound: []});
  const [stayPaymentData, setStayPaymentData] = useState<IStayPaymentData>(stayPaymentInitialData);
  const [bookingData, setBookingData] = useState<IStayBookingPayload>(initialStayBookingPayload);
  const [bookingErrors, setBookingErrors] = useState<IStayBookingPayload>({contact_details: initialContactPayload, passenger_details: []})
  const [bookingTouched, setBookingTouched] = useState<IStayBookingTouchedPayload>({contact_details: initialContactTouchedData, passenger_details: []});
  ...
  const validate = (values: IStayBookingPayload) => {
    // const formValues: IStayBookingPayload = Object.assign(values);
    // validateForErrors(formValues);
    const { bookingErr, errors } = validateForErrors(values, stayDetails.document_required, stayDetails);
  ...
  const setupPassenterDetails = () => {
    ...
    stayDetails.price_summary.map((item: any) => {
      for(let i = 0; i < item.quantity; i++){
  ...
  const setFieldChange = (e: any, values: IStayBookingPayload, setValues: Function, title: string, index: number, inDocument?: boolean) => {
  ...
  const sumbitStayBooking = (values: IStayBookingPayload, control: FormikHelpers<any>) => {
    const payload = updateBookingData(values, stayDetails.document_required);
    setBookingData(payload);
    sendRequest(
      {
        url: "flight/book-flight/" + stayId,
        method: "POST",
        body: payload,
      },
      (res: any) => {
        toast.success('Your stay data has been captured, please make payment to complete booking');
        setStayPaymentData({
          email: values.contact_details.c_email,
          amount: stayDetails.pricing?.payable * 100,
    ...
    <>
      ...
      }
      {
        ...
        <Formik
          ...
          onSubmit={(values, control) => sumbitStayBooking(values, control)}
              ...
              return <form onSubmit={handleSubmit}>
                {
                  ...
                  <div className='preview-sect'>
                    ...
                    <div className=''>
                      <button 
                        className={'stay-button' + (!isValid ? ' inactive' : '')} 
                        onClick={touchAll} 
                        disabled={isSubmitting}
                      >
                        {isSubmitting ? 'Processing...' : 'Book Stay'}
                      </button>
                    </div>
                  </div>
                }
        ...
        </Formik>
        {
          openPayment && 
          <>
            <StayPayment data={stayPaymentData} />
          </>
        }
        </div>
      }
